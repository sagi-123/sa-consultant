...
import { Upload, MapPin, Building2, Users, Calendar, CheckCircle, AlertCircle } from "lucide-react";
...
export default function BusinessOnboarding() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!location.trim()) {
      ...
      toast.error("Please enter a business location.");
      return;
    }
    if (!businessType) {
    ...
    if (!hiringFrequency) {
      toast.error("Please select how frequently you hire.");
      return;
    }
    ...
    // Save profile to localStorage for quick demo state persistence
              ...
              <form onSubmit={handleSubmit} className="space-y-7">
                ...
                <div>
                  ...
                  <Input
                    ...
                    className="h-11 rounded-xl text-foreground font-medium"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder="Enter business location (e.g. New York, NY or Mumbai, MH)"
                      className="h-11 pl-10 rounded-xl text-foreground font-medium"
                      required
                    />
                  </div>
                  {locationError && (
                    <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
                      <AlertCircle className="w-3.5 h-3.5" /> Please select a location with a valid address from the dropdown.
                    </p>
                  )}
                </div>
                  ...
                  <Select value={businessType} onValueChange={setBusinessType}>
                    ...
                    <SelectContent className="rounded-xl">
                      {businessTypeOptions.map((type) => (
                        <SelectItem key={type} value={type} className="rounded-lg cursor-pointer">
                          {type}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  <Select value={hiringGroup} onValueChange={setHiringGroup}>
                    ...
                    <SelectContent className="rounded-xl">
                      {hiringGroupOptions.map((group) => (
                        <SelectItem key={group} value={group} className="rounded-lg cursor-pointer">
                          {group}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  <Select value={hiringFrequency} onValueChange={setHiringFrequency}>
                    ...
                    <SelectContent className="rounded-xl">
                      {hiringFrequencyOptions.map((freq) => (
                        <SelectItem key={freq} value={freq} className="rounded-lg cursor-pointer">
                          {freq}
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                <div className="pt-4">
                  <Button
                    type="submit"
                    className="w-full h-12 text-base font-bold rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg transition-all"
                  >
